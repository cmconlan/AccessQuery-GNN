...
def GNNSimple(x,y,device,edgeIndexNp,edgeWeightsNp,hidden1,hidden2,epochs,trainMask,testMask):
    ...
    edgeWeights2 = torch.tensor(np.expand_dims(edgeWeights,1)).to(device).float()
    ...
    timeEnd = time.time()
    return np.squeeze(model(_x, edgeIndex, edgeWeights)[testMask].cpu().detach().numpy()), timeEnd-timeStart
